...
description: |
  ...
  writes to GitHub directly. Run it repeatedly with the host agent's
  loop or automation runner to drain the queue.
---
...
The cost win: the LLM "reason" step happens in your local agent
session rather than chronicle-server's metered operational LLM key.
...
# 3. REASON (this is where the agent does the actual work — the section
#    below is shaped as instructions to the agent, not Python to execute).
```

## Triage reasoning (instructions to the agent)
...
Run this skill repeatedly with the host's loop or automation runner:

```
$ claude /loop triage-error-queue        # Claude Code
$ codex exec "triage the error queue"    # Codex, from an external shell loop
```

The skill returns "queue empty" cleanly on a 204 from claim, which
the runner should detect and terminate on.

## After the skill completes

Print a one-liner: `<decision> · <root_cause_id> · <github_issue_url>`.
If automation is driving, that's enough; if the operator ran the skill
...
stalls (long agent turn, network blip), heartbeats keep your lease
