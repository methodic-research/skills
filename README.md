...
| `chronicle-bundle-variation` | "bundle my code and run it on a worker", "package this local project as the variation's code", "ship my training to a managed worker" | Zips a local project (incl. `.git` for provenance), registers it as the variation's `code_artifact` input, and creates the variation — so a managed Menlo Park worker pulls the bundle, `pip install`s it, and trains. Bundle alternative to the git-push skills; supersedes "external git repo + ref". |
