...
- **Active scope (org override).** A key acts in its **default scope** — set when the key was minted (personal for user keys; the bound org for agent keys). Scoped list/create endpoints (experiments, datasets, workers, …) require a resolvable scope and filter/attribute by it, so the active scope decides *which org an experiment is created under* and *whose data a listing returns*. When the user asks the agent to work in a **different** org they belong to, set it explicitly before making calls:
  ```python
  chronicle.active_org = "<org-principal-id>"   # sends X-Chronicle-Active-Owner on every request
  ```
  The override must be a scope the key is bound to, or the server rejects the request. Don't invent an org id — take it from the user (or the org's homepage URL `/o/<slug>`). Leaving `active_org` unset uses the key's default, which is the right behavior for the common single-org case.
