...
description: |
  ...
  entry as in_fix. Pair with `/loop` to drain the queue.
---
...
Cost win: the LLM "design + write the fix" step runs against your local
Claude Max subscription, not chronicle's metered API key. The operator
...
# 4. WORK THE FIX (instructions to Claude follow, not Python).
```

## Fix reasoning (instructions to Claude)
...
3. **Verify locally**:
   ...
   - `cargo fmt` per the CLAUDE.md convention
4. **Commit + push + PR**:
   ```python
